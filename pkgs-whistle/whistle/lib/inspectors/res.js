        ...
        readFirstChunk(req, function () {
            ...
            rules.getClientCert(req, function (key, cert, isPfx, cacheKey) {
                rules.getProxy(options.href, options.isPlugin ? null : req, function (err, hostIp, hostPort) {
                    ...
                    var proxyUrl = !options.isPlugin && proxyRule
                    ...
                    rules.resolveHost(req, function (err, ip, port, hostRule) {
                        ...
                        function request(options, serverPort, direct) {
                                ...
                                try {
                                    ...
                                    client.once('socket', function (socket) {
                                        if (socket.connecting || socket._connecting) {
                                            ...
                                            socket.once(isHttpsProxy || isHttps
                                                ? 'secureConnect'
                                                : 'connect', function () {
                            ...
                            var transfer = options.method === 'DELETE' &&
                ...
                pluginMgr.getResRules(req, _res, function () {
                    ...
                    util.parseRuleJson(ruleList, function (headers, cookies, cors, replacement, params, newTrailers) {
                        ...
                        if (data.headers) {
                            ...
                            // @ts-expect-error ts-migrate(2339) FIXME: Property 'headers' does not exist on type '{}'.
                            data.headers, res.rawHeaderNames);
                        }
                                ...
                                util.setHeaders(data, {
                                    'cache-control': noCache
                                        ? /^no-store$/i.test(cache)
                                            ...
                                            : 'no-cache'
                                        : 'max-age=' + maxAge,
                                    expires: new Date(Date.now() + (noCache ? -60000000 : maxAge * 1000)
                        ...
                        if (resRules.attachment) {
                            var attachment = util.getMatcherValue(resRules.attachment) ||
                                util.getFilename(req.fullUrl);
                            util.setHeader(data, 'content-disposition', 'attachment; filename="' +
                        ...
                        util.readInjectFiles(data, function (data) {
                            ...
                            if (data.charset && typeof data.charset == 'string') {
                                ...
                                type =
                                    type ||
                                        mime.lookup(req.fullUrl.replace(/[?#].*$/, ''), 'text/html');
                            ...
                            util.getRuleValue([
                                ...
                                var speedTransform = data.speed || data.delay
                                ...
                                util.getFileWriters([bodyFile, rawFile], function (writer, rawWriter) {
                                    ...
                                    util.delay(util.getMatcherValue(resRules.resDelay), function () {
                                        ...
                                        else if (req._pipePluginPorts.resReadPort ||
