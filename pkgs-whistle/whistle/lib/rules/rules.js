...
var PURE_FILTER_RE = /^(?:excludeFilter|includeFilter):\/\/(statusCode|from|clientIp|clientIP|clientPort|remoteAddress|remotePort|serverIp|serverIP|serverPort|host|re[qs](?:H(?:eaders?)?)?)[.=](.+)$/;
...
var TPL_VAR_RE = /(\$)?\$\{(\{)?(id|reqId|whistle|now|host|port|realPort|realHost|version|url|hostname|query|search|queryString|searchString|path|pathname|clientId|localClientId|ip|clientIp|clientPort|remoteAddress|remotePort|serverIp|serverPort|method|status(?:Code)|reqCookies?|resCookies?|re[qs]H(?:eaders?)?)(?:\.([^{}]+))?\}(\})?/gi;
    ...
    for (var i = 0; (rule = list[i]); i++) {
        ...
        if (rule.isRegExp) {
            ...
            if ((not ? !matchedRes : matchedRes) && checkFilter() && --index < 0) {
                ...
                (result = getExactRule(getRelativePath(pattern, curUrl, rule.matcher)))) {
                    return result;
                }
            }
        }
...
proto.getHost = function (req, pluginRulesMgr, rulesFileMgr, headerRulesMgr) {
    ...
    req.curUrl = curUrl;
    var filter = {};
    var filterHost = ignoreHost(req, this, filter) ||
...
proto.resolveBodyFilter = function (req) {
    return req.isPluginReq
        ...
        : getRule(req, req._bodyFilters || this._rules._bodyFilters, null, null, true);
};
