...
var REMOTE_RULES_RE = /^\s*@(`?)(whistle\.[a-z\d_\-]+(?:\/[^\s#]*)?|(?:https?:\/\/|[a-z]:[\\/]|~?\/)[^\s#]+|\$(?:whistle\.)?[a-z\d_-]+[/:][^\s#]+)\s*?\1(?:#.*)?$/im;
...
pluginMgr.on('updateRules', function () {
    ...
    Object.keys(allPlugins)
        ...
        var p1 = allPlugins[a];
        var p2 = allPlugins[b];
        return (util.compare(p1.priority, p2.priority) ||
...
function showVerbose(oldData, newData) {
    ...
    uninstallData &&
        Object.keys(uninstallData).forEach(function (name) {
            console.log(colors.red(util.formatDate(new Date(uninstallData[name].mtime)) +
                ' [uninstall plugin] ' +
                name.slice(0, -1)));
        });
    installData &&
        Object.keys(installData).forEach(function (name) {
            console.log(colors.green(util.formatDate(new Date(installData[name].mtime)) +
                ' [install plugin] ' +
                name.slice(0, -1)));
        });
    updateData &&
        Object.keys(updateData).forEach(function (name) {
            console.log(colors.yellow(util.formatDate(new Date(updateData[name].mtime)) +
                ' [update plugin] ' +
                name.slice(0, -1)));
        });
}
            ...
            getPlugin(function (result) {
                readPackages(result, function (_plugins) {
                    ...
                    pluginNames.forEach(function (name) {
                        ...
                        else if (newPlugin.path != plugin.path ||
        ...
        authReq(enableAuth, ports, req, options, function (forbidden) {
            if (forbidden) {
                ...
                if (noTunnel && req._authHtmlUrl) {
                    mgr.parse('* ignore://!method|!file|!http|!https method://get ' +
                        req._authHtmlUrl);
                }
...
pluginMgr.updatePluginRules = function (name) {
    name &&
        httpMgr.forceUpdate('http://127.0.0.1:' + config.port + '/whistle.' + name + '/');
};
