...
function execScriptSync(script, context) {
    try {
        if ((script = getScript(script))) {
            CONTEXT.console = {};
            ['fatal', 'error', 'warn', 'info', 'log', 'debug'].forEach(function (level) {
...
exports.setClientId = function (headers, enable, disable, clientIp, isInternalProxy) {
    ...
    if (enable.clientId ||
        ...
        if ((enable.multiClient || isInternalProxy) &&
            !enable.singleClient &&
            !disable.multiClient) {
...
function readRuleList(rule, callback, isJson, charset, isHtml, req) {
    ...
    rule.list.forEach(function (r, i) {
        if (isJson) {
            ...
            if (value) {
                var json = isCors && GEN_URL_RE.test(value)
                    ? { origin: value.trim() }
